...
void TouchScreenGUI::applyContextControls(const PointedThing &pointed, const TouchControlHint &hint) {
	...
	case POINTEDTHING_NOTHING:
		mode = hint.pointed_nothing;
		break;
	case POINTEDTHING_NODE:
		mode = hint.pointed_node;
		break;
	case POINTEDTHING_OBJECT:
		mode = hint.pointed_object;
		break;
	}
	...
	case TapState::ShortTap:
		if (mode == TouchControlMode::ShortDigLongPlace) {
			if (!m_dig_pressed) {
